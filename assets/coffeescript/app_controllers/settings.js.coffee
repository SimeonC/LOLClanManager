SettingsController = class SettingsController
	constructor: ($scope, $routeParams, data, settings, sharedFuncs, $http, sockets) ->
		...
		$scope.data = data
		$scope.lastformula = $scope.formula = $scope.settings.aggregateFormula
		# for when it isn't loaded by the time we get in
		$scope.$watch 'settings.loading', -> $scope.lastformula = $scope.formula = $scope.settings.aggregateFormula
		$scope.allPlayers = []
		$scope.players = {}
